...
DROP FUNCTION daily_profit;
--DROP TRIGGER product_price;
--DROP INDEX emp_FnameLname_index;
...
INSERT INTO supplier
VALUES (0787654321,'Marios','Georgiou','Kolejni 5, Brno, 61200',1010101000,1234567890);
INSERT INTO product
...
INSERT INTO product
VALUES (2020202023,'2','5','10',1234567890);
INSERT INTO product
VALUES (2020202024,'1','10','10',1234567890);
INSERT INTO product
VALUES (2020202025,'1','12','15',1234567890);
...
INSERT INTO department
VALUES ('Management',1234567890);
INSERT INTO employee
VALUES (2222222223,'Athina','Zisimou',3000,99000000,'Management');
INSERT INTO employee
...
-- It prevents the user from entering
-- a price in a product that is below zero
...
--Tests the trigger by inserting a product with
--price '-4'
INSERT INTO product
VALUES (2020202025,'3','-4','30',1234567890);

--Tests the trigger by updating the price of the product with ID = 2020202020
--price '-5'
UPDATE product 
SET price = '-5' 
WHERE productID = 2020202022;
...
set autotrace on;
-- we set it on in order to see the difference
-- the index has on the execution 

-- We will update the name of employee Leonidas to Leo before the index
UPDATE employee 
SET firstName = 'Leo' 
WHERE firstName = 'Leonidas';

set autotrace off;

--Create index for employees firstname and lastname
CREATE INDEX emp_FnameLname_index ON employee(firstName, lastName);

set autotrace on;

-- We will update the name of employee Leonidas to Leo after the index
UPDATE employee 
SET firstName = 'Leontarious' 
WHERE firstName = 'Leo';
...
--We will do a select of all the procudt of type 1 to take the "before" measurement
SELECT *
FROM product
WHERE typeofprod='1';
...
--Creatin index for pruducts type
CREATE INDEX product_typoOfProduct_index ON product(typeOfProd);

set autotrace on;
-- we set it on in order to see the difference
-- the index has on the execution 

--We will do a select of all the procudt of type 1 to take the "after" measurement
SELECT *
FROM product
WHERE typeofprod='1';
...
--We will do the same select to take the "after" measurement
SELECT *
FROM supplier
WHERE firstName='Marios' AND lastName='Pafitis';

set autotrace off;

--Create index for suppliers firstname and lastname
CREATE INDEX sup_FnameLname_index ON supplier(firstName, lastName);
...
CREATE PROCEDURE delete_emp (empID NUMBER)  AS
...
-- Test procedure remove employee
-- emplyee table before the deletion
SELECT *
FROM employee;

-- Delete the employee with employeeID 2222222220
EXECUTE delete_emp(2222222220);

-- emplyee table after the deletion
SELECT *
FROM employee;
...
---Get the total profit for a supermarket
CREATE FUNCTION daily_profit(strID IN NUMBER) 
   RETURN NUMBER 
   IS total NUMBER(11,2);
   BEGIN 
      SELECT DISTINCT SUM(money) 
      INTO total 
      FROM cash_register 
      WHERE storeID = strID; 
      RETURN(total); 
...
-- Test function daily profit of a supermarket
SELECT daily_profit(1234567890) FROM supermarket;